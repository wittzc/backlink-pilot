...
> 做「做不做」决策时的领域输入（外链领域有哪些打法全集）→ 见 [research/外链建设方法论全景.md](research/外链建设方法论全景.md)。
>
...
| **提交侧单日总量护栏（限"单日提交数 < N"）** | 护错了对象。方法论「单日 < 5」红线指的是**已生效、被搜索引擎识别的外链**，工具能控的只有**提交动作**——两者隔着"目录站审核（拒/排队几天到几周）→ 页面公开 → Google 索引"整条漏斗，目录提交大多当天不收录、被审核流程天然错峰摊平。「提交暴增→外链暴增→link scheme」的因果链在目录场景里已被审核漏斗打断。拿提交数近似外链增速：护不住真风险（真会暴增的是"提交即生效"的快渠道，属渠道属性问题非总数问题），还误伤健康操作（冷启动一天铺 20 个提交、最终只几个收录且分散数周上线，是健康的却被卡死）。结论：节奏护栏不加在提交侧；真正该补的是**收录侧测量**（verify 已提交链接是否上线 + dofollow），见 FEATURES。 | （指标错位） |
