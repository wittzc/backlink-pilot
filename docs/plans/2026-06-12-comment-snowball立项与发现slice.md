...
**P0-1 / P0-3 已验证通过**(2026-06-12,用户登录态取 basketball-stars.io 外链 20 条样本):

- **P0-1 评论可公开 ✅,且修正认知**:20 条 referring page 绝大多数是**博客评论**,且全是**毫不相关 niche**(食谱 / 家居 / 手工 / 旅游博客,DR 37-85)。anchor 暴露纯灌水(如素食食谱评论区塞 *"What is the maple syrup alternative? basketball stars"*)。**纯灌水 + 零相关照样公开被收录**。
- **关键认知修正**:方法论说「评论要相关、有价值」——但小游戏赛道真实打法是「**数量 + 找软审核站 + 不管相关性**」。挂一批无所谓,几千个里公开几百个就够。**目标站筛选标准不是 niche 相关,而是「审核松」**(食谱博主不盯评论 spam)。这解释了 nofollow 实测为何失败:addfamilyphoto 在严审核 family 博客认真写相关评论(3-11 个)→ 0 公开;basketball-stars 在软审核食谱博客纯灌水(几千个)→ DR 46。差别是**目标站选择 + 数量**,不是 nofollow 技术。
- **P0-3 滚雪球燃料 ✅**:visitwiltshire 一条 anchor 含 *"games like Moto X3M and Football Masters. In Basketball Stars..."*——**一条评论 3 个同行游戏站**。且这批软审核博客被多个游戏站共用,评论区能捞到更多游戏站 → 滚雪球成立。

**Phase 0 结论:三假设全过 → 项目 ROI 赌注成立,可进 Slice 1。** 待验细节:概览 91% dofollow vs 博客评论默认 nofollow 的矛盾(可能来自论坛/profile 或软审核站关了 nofollow),Slice 1 顺带验。

> **目标站筛选标准(Phase 0 产出,直接进 Slice 1 设计)**:发现的不是「niche 相关博客」,而是「**被多个游戏/工具站反复刷过评论的软审核博客**」——被刷过 = 审核松 = 你也能发。这是 S1-4 判定「可发布文章」的核心信号。
